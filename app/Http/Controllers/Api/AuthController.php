...
class AuthController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        ...
        $user = User::query()->where('national_id', $credentials['national_id'])->first();
        ...
        if ($user->isFamilyHead() && $user->camp_id !== null) {
            ...
            if ($camp && $camp->familiesHardBlocked()) {
                throw ValidationException::withMessages([
                    'national_id' => ['لا يمكن الدخول: انتهى اشتراك المخيم وبعد فترة السماح لم يُجدَّد الدفع ('.(int) config('subscription.monthly_amount_ils', 15).' شيكل شهرياً). تواصل مع إدارة المخيم أو منصة تَكافل.'],
                ]);
            }
        }
        ...
        if ($user->isFamilyHead() && $user->camp_id !== null) {
            ...
            if ($camp) {
                $data['subscription'] = [
                    ...
                    'monthly_amount_ils' => (int) config('subscription.monthly_amount_ils', 15),
